...
public class QuarkusLikeORMUnitTestCase extends BaseCoreFunctionalTestCase {
	...
	public void hhh123Test() throws Exception {
		...
		try(Session s = openSession()) {
			...
			s.persist(new ExampleEntity("id-1", Set.of("element-1")));
			s.persist(new ExampleEntity("id-2", Set.of("element-2")));
			s.persist(new ExampleEntity("id-3", Set.of("element-3")));
			tx.commit();
		}
		try(Session s = openSession()) {
			Transaction tx = s.beginTransaction();
			var entities = s.createQuery("select e from ExampleEntity e", ExampleEntity.class).getResultList();
			s.createQuery("select e from DummyEntity e where 1 = 0", DummyEntity.class).getResultList();
